...
namespace WindowsFormsApp3
{
    ...
    public partial class Form12 : Form
    {
        ...
        public void fillItemPageBook()
        {
            ...
            if (ItemType == 3)
            {
                SqlCommand cmd = new SqlCommand("Select * from TXTL where TXName=@TXName", con);
                cmd.Parameters.AddWithValue("@TXName", listView1.SelectedItems[0].Text);
                try
                {
                    ...
                    if (dr.Read())
                    {
                        f4.richTextBox1.Text = (dr["TXDSC"].ToString());
                        f4.label2.Text = (dr["TXPrice"].ToString());
                        f4.label1.Text = (dr["TXName"].ToString());
        ...
        public void fillItemImage() {
            ...
            if (ItemType==2)
            {
                ...
                var ds = new DataSet();
                da.Fill(ds, "Tec");
                int count = ds.Tables["Tec"].Rows.Count;

                if (count > 0)
                {
                    var data = (Byte[])ds.Tables["Tec"].Rows[count - 1]["TImage"];
                    var stream = new MemoryStream(data);
                    f4.pictureBox1.Image = Image.FromStream(stream);
                }
            ...
            if (ItemType == 3)
            {
                ...
                var ds = new DataSet();
                da.Fill(ds, "TXTL");
                int count = ds.Tables["TXTL"].Rows.Count;

                if (count > 0)
                {
                    var data = (Byte[])ds.Tables["TXTL"].Rows[count - 1]["TXImage"];
                    var stream = new MemoryStream(data);
                    f4.pictureBox1.Image = Image.FromStream(stream);
                }
